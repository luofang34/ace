...
pub(crate) fn estimate(
    ...
    let speed = cruise_speed_m_s(scenario)?;
    let mass_ratio = (initial_mass / final_mass).ln();
    let (range_m, method) = match &scenario.engine {
        ...
        EngineProfile::Turbofan(engine) => {
            let tsfc_kg_n_s = engine.tsfc_cruise_kg_n_hr / 3600.0;
            (
                speed / (GRAVITY_M_S2 * tsfc_kg_n_s) * lift_to_drag_ratio * mass_ratio,
                "jet Breguet range with cruise TSFC",
            )
        }
    };
...
fn cruise_speed_m_s(scenario: &ResolvedScenario) -> AexResult<f64> {
    ...
    Ok(mach * atmosphere.speed_of_sound_m_s)
}
